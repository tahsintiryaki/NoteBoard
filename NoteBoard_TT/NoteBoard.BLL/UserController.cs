...
namespace NoteBoard.BLL
{
    public class UserController
    {
        ...
        public User GetByLogin(string username,string password)
        {
            ...
            User user = users.Where(x => x.IsActive && x.UserName == username).SingleOrDefault();
            if (user !=null)
            {
                Password pass = user.Passwords.Where(x => x.IsActive && x.PasswordText == password).SingleOrDefault();
                if (pass==null)
                {
                    return null;
                }
